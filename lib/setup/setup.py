...
from helper import blink_led, led_lock, wifi_lock
...
def setup_new_config(logger):
    ...
    if not wifi_lock.locked():
        with wifi_lock:
            ...
            wlan = WLAN(mode=WLAN.AP, ssid=config.get_config("device_name") + unique_id)
            ...
            wlan.ifconfig(
                id=1,
                config=("192.168.4.10", "255.255.255.0", "192.168.4.1", "192.168.4.1"),
            )

            logger.info(
                "Access point turned on as {}".format(
                    config.get_config("device_name") + unique_id
                )
            )
            logger.info("Configuration website can be accessed at 192.168.4.10")

            address = socket.getaddrinfo("0.0.0.0", 80)[0][
                -1
            ]  # Accept stations from all addresses
            sct = socket.socket()  # Create socket for communication
            sct.settimeout(
                int(float(config.get_config("config_timeout")) * 60)
            )  # session times out after x seconds
...
def process_data(received_data, logger):
    ...
    if first_index != -1 and last_index != -1:
        config_json_str = received_data[
            (first_index + len("json_str_begin")) : last_index
        ]
        new_config_dict = {
            "LORA": "OFF"
